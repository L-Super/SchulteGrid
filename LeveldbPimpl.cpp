...
LeveldbPimpl::LeveldbPimpl() : dbName("./data") { OpenDB(dbName); }

LeveldbPimpl::~LeveldbPimpl() { delete db; }
...
std::list<std::string> LeveldbPimpl::GetAllData() {
  ...
  for (it->SeekToFirst(); it->Valid(); it->Next()) {
    //		spdlog::info("key: {} value: {}", it->key().ToString(),
    // it->value().ToString());
    list.emplace_back(it->value().ToString());
  }
  // 必须要delete，否则Assertion failed
  delete it;
  return list;
}
