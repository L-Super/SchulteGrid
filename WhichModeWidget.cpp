	  ...
	  radioBtnGroup(new QButtonGroup())
{
	...
	connect(ui->retrunBtn, &QPushButton::clicked, [this]()
	{
		emit btnClickedSignal(MenuWidgetSignal);
	});
	connect(ui->enterBtn, &QPushButton::clicked, this, &WhichModeWidget::EnterPushButtonClicked);
}

WhichModeWidget::~WhichModeWidget()
{
	delete ui;
}
...
void WhichModeWidget::EnterPushButtonClicked()
{
	emit btnClickedSignal(GameWidgetSignal);
}
